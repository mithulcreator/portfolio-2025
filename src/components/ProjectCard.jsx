...
export default function ProjectCard({ project }) {
  ...
  return (
    <Link to={`/project/${project.slug.current}`} className="group">
      <div className="relative overflow-hidden rounded-lg bg-zinc-800 transition-all duration-300 hover:shadow-xl">
          ...
          {project.tools && project.tools.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-3">
              {project.tools.map((tool, index) => (
                <span
                  key={index}
                  className="px-2 py-1 text-xs font-medium bg-zinc-700 text-zinc-300 rounded-full"
                >
                  {tool}
                </span>
              ))}
